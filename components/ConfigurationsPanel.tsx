...
export const ConfigurationsPanel: React.FC<ConfigurationsPanelProps> = ({
  ...
  return (
    <div className="glass-panel p-6 rounded-[40px] shadow-xl min-h-[600px] relative overflow-hidden bg-white/80 backdrop-blur-xl">
      ...
      {/* Toast Feedback */}
      {feedback && (
        <div className={`absolute top-6 left-1/2 -translate-x-1/2 px-6 py-3 rounded-2xl text-xs font-black shadow-2xl z-50 flex items-center gap-3 ${feedback.type === 'success' ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[600px]">
                ...
                <div className="lg:col-span-1 flex flex-col gap-4">
                    <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => setEditingContract({ id: `new_${Date.now()}`, Status_Contrato: 'Ativo', Tipo_Servico: 'Agency' })} className="p-2 bg-indigo-600 text-white rounded-xl shadow-lg hover:bg-indigo-700 transition-all"><Plus size={18}/></button>
                    </div>
                    ...
                    <div className="overflow-y-auto custom-scrollbar space-y-2 pr-2">
                        {filteredContracts.map(c => (
                            <div key={c.id} onClick={() => setEditingContract(c)} className={`cursor-pointer p-4 rounded-2xl border transition-all hover:scale-[1.02] active:scale-95 ${editingContract?.id === c.id ? 'bg-slate-800 border-slate-800 ring-2 ring-indigo-400' : 'bg-white border-slate-100 hover:border-indigo-200 hover:shadow-md'}`}>
                                <div className="flex justify-between items-start mb-2">
                                    <h4 className={`font-bold text-sm ${editingContract?.id === c.id ? 'text-white' : 'text-slate-800'}`}>{c.Cliente}</h4>
                                    <span className={`text-[9px] px-2 py-0.5 rounded-md font-black uppercase ${c.Status_Contrato === 'Ativo' ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'}`}>{c.Status_Contrato}</span>
                                </div>
                                <div className="flex gap-2">
                                    <span className={`text-[9px] font-bold uppercase px-2 py-1 rounded-lg ${c.Tipo_Servico === 'UI-Z' ? 'bg-indigo-500 text-white' : 'bg-slate-200 text-slate-500'}`}>{c.Tipo_Servico || 'Agency'}</span>
                                    {c.Tipo_Servico === 'UI-Z' && <span className="text-[9px] font-mono text-indigo-300 pt-1">R$ {c.UIZ_Valor_Mensal}/mês</span>}
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    {editingContract ? (
                        <div className="animate-fade-in h-full flex flex-col">
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-6">
                                <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
                                    <div className="grid grid-cols-2 gap-4 mb-4">
                                        <div className="col-span-2 md:col-span-1">
                                            ...
                                            <input type="text" value={editingContract.Cliente || ''} onChange={e => setEditingContract({...editingContract, Cliente: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-indigo-500/20" autoFocus />
                                        </div>
                                        <div>
                                ...
                                <div className={`p-5 rounded-3xl shadow-sm border transition-all ${editingContract.Tipo_Servico === 'UI-Z' ? 'bg-indigo-50 border-indigo-100' : 'bg-white border-slate-100'}`}>
                                    <h4 className={`text-xs font-black uppercase mb-4 ${editingContract.Tipo_Servico === 'UI-Z' ? 'text-indigo-600' : 'text-slate-400'}`}>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[600px]">
                 ...
                 <div className="lg:col-span-1 flex flex-col gap-4">
                     <div className="flex justify-between items-center mb-2">
                        <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Competência: <span className="text-indigo-600">{selectedMonth}</span></h3>
                     </div>
                     <div className="overflow-y-auto custom-scrollbar space-y-2 pr-2">
                        {contracts.filter(c => c.Status_Contrato === 'Ativo').map(c => {
                             ...
                             if (hasResult) {
                                 if (hasResult.Status_Pagamento === 'Pago') statusStyle = 'bg-emerald-50 border-emerald-200 text-emerald-700';
                                 else if (hasResult.Status_Pagamento === 'Atrasado') statusStyle = 'bg-rose-50 border-rose-200 text-rose-700';
                                 else statusStyle = 'bg-amber-50 border-amber-200 text-amber-700';
                             }
                             ...
                             return (
                                 <button 
                                    ...
                                    className={`w-full text-left p-4 rounded-2xl border transition-all flex justify-between items-center ${editingResult?.contractId === c.id ? 'ring-2 ring-indigo-500' : ''} ${statusStyle}`}
                                 >
                                    <div>
                                    ...
                                    <div className="col-span-2 md:col-span-1">
                                        ...
                                        <select 
                                            ...
                                            className={`w-full p-4 rounded-2xl text-sm font-black outline-none appearance-none cursor-pointer border-r-[16px] border-r-transparent ${
                                                editingResult.Status_Pagamento === 'Pago' ? 'bg-emerald-100 text-emerald-700' :
